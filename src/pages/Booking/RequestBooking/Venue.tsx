...
function VenueBookings({ userId }: VenueBookingsProps) {
  const [venueBookings, setVenueBookings] = useState<VenueBooking[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [currentPage, setCurrentPage] = useState(1);
  const [sortConfig, setSortConfig] = useState<{ key: keyof VenueBooking; direction: 'asc' | 'desc' } | null>(null);
  ...
  useEffect(() => {
    if (userId) fetchVenueBookings();
  }, [userId]);

  const handleSort = (key: keyof VenueBooking) => {
  ...
  const filteredData = venueBookings
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  {['id', 'venue_id', 'start_date', 'end_date', 'status', 'service', 'name'].map((header) => (
                    <th
                      key={header}
                      className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600"
                      onClick={() => handleSort(header as keyof VenueBooking)}
                    >
                      {header.replace('_', ' ')}
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {paginatedData.map((booking) => (
                  <tr key={booking.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{booking.venue_id}</td>
