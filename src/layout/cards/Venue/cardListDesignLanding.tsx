...
const Card: React.FC<CardProps> = ({ VenueName, PlaceName, capacity, image, rating, Price, venueType }) => {
    ...
    return (
        <div className="relative group border border-gray-400/20 overflow-hidden cursor-pointer w-full flex flex-col h-full">
            <span
            ...
            {/* Image Section */}
            <div className="relative z-10 flex-shrink-0 h-[200px] sm:h-[250px]">
                <img
            ...
            {/* Content Section */}
            <div className="px-4 py-6 relative z-10 flex flex-col flex-grow">
                <div>
                ...
                <p className="text-gray-500 font-sofia tracking-wide text-sm group-hover:text-white transition-colors duration-300 mb-2">Capacity: {capacity || 'N/A'}</p>
                <p className="text-gray-500 font-sofia tracking-wide text-sm group-hover:text-white transition-colors duration-300 mb-2">Price-Range: {Price !== null ? Price : 'N/A'}</p>
                <p className="text-gray-500 font-sofia tracking-wide text-sm group-hover:text-white transition-colors duration-300 mb-2">
                    {venueType.length > 0 ? (
                        venueType.slice(0, 3).map((type, index) => (
                            <span key={index} className="inline-block bg-sky-200/10 rounded-full px-3 py-1 text-sm font-semibold text-sky-700 mr-2 mb-2">
                                {type}
                            </span>
                        ))
                    ) : 'N/A'}
                    {venueType.length > 3 && (
                        <span className="inline-block bg-sky-200/10 rounded-full px-3 py-1 text-sm font-semibold text-sky-700 mr-2 mb-2">...</span>
                    )}
                </p>
