      ...
      >
        ...
        <nav>
          <ul className={`space-y-2 ${isCollapsed ? 'mt-[4rem]' : 'mt-0'}`}>
            <SidebarItem to="/Supplier-Dashboard/Home" icon={<GoHome className={`text-2xl ${isCollapsed ? 'mx-auto ' : ''}`} />} label="Home" isCollapsed={isCollapsed} />
            <SidebarItem to="/Supplier-Dashboard/Company" icon={<MdOutlineManageAccounts className={`text-2xl ${isCollapsed ? 'mx-auto' : ''}`} />} label="User Management" isCollapsed={isCollapsed} />
            <SidebarItem to="/Supplier-Dashboard/Create-User" icon={<MdPeople className={`text-2xl ${isCollapsed ? 'mx-auto' : ''}`} />} label="Attendee Management" isCollapsed={isCollapsed} />
            <SidebarItem to="/Supplier-Dashboard/Financial-Overview" icon={<MdAttachMoney className={`text-2xl ${isCollapsed ? 'mx-auto' : ''}`} />} label="Financial Overview" isCollapsed={isCollapsed} />
            <SidebarItem to="/Supplier-Dashboard/Supplier-List" icon={<MdAssignment className={`text-2xl ${isCollapsed ? 'mx-auto' : ''}`} />} label="Supplier" isCollapsed={isCollapsed} />
            <SidebarItem to="/Supplier-Dashboard/Analytics" icon={<MdAnalytics className={`text-2xl ${isCollapsed ? 'mx-auto' : ''}`} />} label="Analytics" isCollapsed={isCollapsed} />
            <SidebarItem to="/Supplier-Dashboard/Settings" icon={<MdSettings className={`text-2xl ${isCollapsed ? 'mx-auto' : ''}`} />} label="Settings" isCollapsed={isCollapsed} />
            <SidebarItem to="/Supplier-Dashboard/Support-Help" icon={<MdHelpOutline className={`text-2xl ${isCollapsed ? 'mx-auto' : ''}`} />} label="Support & Help" isCollapsed={isCollapsed} />
          </ul>
        </nav>
      </aside>
    </>
